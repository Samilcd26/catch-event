...
class IImagePicker extends StatefulWidget {
  const IImagePicker({super.key, required this.title});
  final String title;
...
class _IImagePickerState extends State<IImagePicker> {
  Future<File?> _cropImage({required File imageFile}) async {
    CroppedFile? croppedImage =
        await ImageCropper().cropImage(sourcePath: imageFile.path, aspectRatio: const CropAspectRatio(ratioX: 16, ratioY: 9));
